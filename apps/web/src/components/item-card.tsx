  ...
  return (
    <div className="border border-gray-700 rounded-lg p-4 hover:shadow-md transition-shadow bg-gray-800">
      <div className="flex gap-4">
        <Link href={`/item/${item.id}`} className="flex gap-4 flex-1">
          {item.thumbnail && (
            <img
              src={item.thumbnail}
              alt={item.title}
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-lg mb-1 text-gray-100">{item.title}</h3>
            {item.channel && (
        ...
        <button
          ...
          className="self-start px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700"
        >
          Delete
        </button>
      </div>
